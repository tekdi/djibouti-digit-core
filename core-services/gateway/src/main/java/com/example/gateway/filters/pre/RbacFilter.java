...
import static com.example.gateway.filters.pre.helpers.AuthPreCheckFilterHelper.AUTH_TOKEN_RETRIEVE_FAILURE_MESSAGE;
...
public class RbacFilter implements GlobalFilter, Ordered {
    ...
    public RbacFilter(ModifyRequestBodyGatewayFilterFactory modifyRequestBodyFilter, RbacFilterHelper rbacFilterHelper,
            CommonUtils commonUtils, ObjectMapper objectMapper, MultiStateInstanceUtil centralInstanceUtil,
            RestTemplate restTemplate, ApplicationProperties applicationProperties) {
        ...
        if (Boolean.TRUE.equals(rbacFlag)) {
            if (contentType == null
                ...
                return modifyRequestBodyFilter.apply(new ModifyRequestBodyGatewayFilterFactory.Config()
                        .setRewriteFunction(Map.class, Map.class, rbacFilterHelper)).filter(exchange, chain);
            }
    ...
    private void isIncomingURIInAuthorizedActionList(ServerWebExchange exchange) {
        ...
        if (centralInstanceUtil.getIsEnvironmentCentralInstance()
                && StringUtils.isEmpty(exchange.getAttributes().get(TENANTID_MDC))) {
        ...
        AuthorizationRequest request = AuthorizationRequest.builder().roles(new HashSet<>(user.getRoles()))
                .uri(requestUri).tenantIds(tenantIds).build();

        boolean isUriAuthorised = isUriAuthorized(request, exchange);

        if (!isUriAuthorised) {
            throw new CustomException(HttpStatus.UNAUTHORIZED.toString(),
    ...
    private boolean isUriAuthorized(AuthorizationRequest authorizationRequest, ServerWebExchange exchange) {
        ...
        try {
            ...
            ResponseEntity<Void> responseEntity = restTemplate
                    .postForEntity(applicationProperties.getAuthorizationUrl(), httpEntity, Void.class);
