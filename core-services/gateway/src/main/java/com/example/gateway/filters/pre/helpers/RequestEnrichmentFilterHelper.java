...
public class RequestEnrichmentFilterHelper implements RewriteFunction<Map, Map> {
    ...
    public RequestEnrichmentFilterHelper(MultiStateInstanceUtil centralInstanceUtil, CommonUtils commonUtils,
    ...
    private void addPassThroughGatewayHeader(ServerWebExchange exchange) {
        exchange.getRequest().mutate().headers(httpHeaders -> {
            httpHeaders.add(GatewayConstants.PASS_THROUGH_GATEWAY_HEADER_NAME,
